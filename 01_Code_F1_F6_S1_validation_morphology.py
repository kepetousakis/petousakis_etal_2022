...
filepath = './Data_F1_F6_S1_dendrite_properties.tsv'
...
if __name__ == "__main__":
	...
	if not os.path.exists('./Data_F1_F6_S1_processed.pickle'):
		na_data = util.pickle_load('./Data_F1_F6_S1_Na_responses.pickle')
		nmda_data = util.pickle_load('./Data_F1_F6_S1_NMDA_noNa_responses.pickle')
		print('Loaded raw data...')
		process_traces = True
	else:
		(na_thresholds,nmda_thresholds,na_maxima,na_maxima_backup,nmda_na_maxima,dendrite_NRLEs,traces_to_visualize_basal,traces_to_visualize_apical) = util.pickle_load('./Data_F1_F6_S1_processed.pickle')
...
fig = plt.figure(); plt.bar(xA, naTA, width=-0.45, color='b', align='edge'); plt.bar(xA, nmdaTA, width=0.45, color='r', align='edge'); plt.ylim([0,110]); fig.set_tight_layout(True)
...
fig = plt.figure(); plt.bar(xB, naTB, width=-0.45, color='b', align='edge'); plt.bar(xB, nmdaTB, width=0.45, color='r', align='edge'); plt.ylim([0,110]); fig.set_tight_layout(True)
plt.legend(['Sodium spike threshold','NMDA spike threshold'])
...
#%% Diameter vs threshold (Figure 6A)
thresholds = na_thresholds
diameters = [dendrite_information[x]['diameter'] for x in dendrite_names]

fig = plt.figure(); fig.set_tight_layout(True)
plt.scatter(diameters[0:7], thresholds[0:7],c='r',label='Basal')
plt.scatter(diameters[7:], thresholds[7:],c='k',label='Apical')
plt.xlabel('Diameter (um)'); plt.ylabel('Sodium spike threshold (#syn)')
plt.title('Figure 6A')
plt.scatter(diameters[36+7], thresholds[36+7],c='g',label='Atypical')
plt.scatter(diameters[29+7], thresholds[29+7],c='g')
plt.scatter(diameters[18+7], thresholds[18+7],c='g')
fig.set_tight_layout(True)
figManager = plt.get_current_fig_manager()
figManager.window.showMaximized()
plt.legend()
...
#%% Distance from soma vs threshold (Figure 6B)
thresholds = na_thresholds
distances = [dendrite_information[x]['distance'] for x in dendrite_names]

fig = plt.figure(); fig.set_tight_layout(True)
plt.scatter(distances[0:7], thresholds[0:7],c='r',label='Basal')
plt.scatter(distances[7:], thresholds[7:],c='k',label='Apical')
plt.xlabel('Distance from soma (um)'); plt.ylabel('Sodium spike threshold (#syn)')
plt.title('Figure 6B')
plt.scatter(distances[36+7], thresholds[36+7],c='g',label='Atypical')
plt.scatter(distances[29+7], thresholds[29+7],c='g')
plt.scatter(distances[18+7], thresholds[18+7],c='g')
fig.set_tight_layout(True)
figManager = plt.get_current_fig_manager()
figManager.window.showMaximized()
plt.legend()
...
#%% Scatter plot of electrotonic constant vs sodium spiking threshold per dendritic segment, with linear fit (Figure 6C)
thresholds = na_thresholds
ELPs = [dendrite_information[x]['ELP'] for x in dendrite_names]

banned = [0,1,2,3,4,21,18,29,36]  # IDs of trunk and atypical dendrites

fig = plt.figure()
plt.scatter(ELPs[0:7], thresholds[0:7],c='r',label='Basal')
plt.scatter(ELPs[7:], thresholds[7:],c='k',label='Apical')
plt.xlabel('Dendritic electrotonic constant'); plt.ylabel('Threshold (# synapses)')
plt.title('Figure 6C')
plt.scatter(ELPs[36+7], thresholds[36+7],c='g',label='Atypical')
plt.scatter(ELPs[29+7], thresholds[29+7],c='g')
plt.scatter(ELPs[18+7], thresholds[18+7],c='g')
plt.scatter(ELPs[0+7], thresholds[0+7],c='c',label='Trunk')
plt.scatter(ELPs[1+7], thresholds[1+7],c='c')
plt.scatter(ELPs[2+7], thresholds[2+7],c='c')

plt.scatter(ELPs[3+7], thresholds[3+7],c='c')
plt.scatter(ELPs[4+7], thresholds[4+7],c='c')
plt.scatter(ELPs[21+7], thresholds[21+7],c='c')
plt.legend()

x_b = np.array(ELPs[0:7]).reshape(-1, 1)
y_b = np.array(na_thresholds[0:7]).reshape(-1, 1)

x_a = np.array([x for i,x in enumerate(ELPs[7:]) if ELPs[i+7] < np.min(np.array(ELPs[7:])[banned])]).reshape(-1, 1)
y_a = np.array([x for i,x in enumerate(na_thresholds[7:]) if ELPs[i+7] < np.min(np.array(ELPs[7:])[banned])]).reshape(-1, 1)

model_linear_b = LinearRegression().fit(x_b, y_b)
model_linear_a = LinearRegression().fit(x_a, y_a)

Rsq_b = model_linear_b.score(x_b, y_b)
Rsq_a = model_linear_a.score(x_a, y_a)

intc_b = model_linear_b.intercept_
intc_a = model_linear_a.intercept_
slope_b = model_linear_b.coef_
slope_a = model_linear_a.coef_

xrange = np.linspace(0,1,200)

line_a = lambda x: (slope_a * x) + intc_a
line_b = lambda x: (slope_b * x) + intc_b

point_cutoff = np.min(np.array(ELPs[7:])[banned])

plt.axvline(point_cutoff, c='grey', ls='--', label='Cutoff')

plt.plot(xrange, [line_a(x)[0][0] for x in xrange], ls='--', c='k', label='Apical fit')
plt.plot(xrange, [line_b(x)[0][0] for x in xrange], ls='--', c='r', label='Basal fit')
plt.ylim([0,100])
fig.set_tight_layout(True)
figManager = plt.get_current_fig_manager()
figManager.window.showMaximized()
